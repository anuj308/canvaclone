...
function UploadPanel(){
    ...
    const handleFileUpload = async(e)=>{
        ...
        try {
            ...
            console.error('Error while uploading the file:', error?.message || error);
        } finally{
    ...
    return (
        <div className="h-full p-1 overflow-y-auto">
            <div className="flex gap-2">
                <Label className={`w-full flex items-center justify-center gap-2 py-3 px-4 bg-purple-600 hover:bg-purple-700 text-white rounded-md cursor-pointer h-12 font-medium transition-colors ${isUploading ? 'opacity-70 cursor-not-allowed': ''}`}>
